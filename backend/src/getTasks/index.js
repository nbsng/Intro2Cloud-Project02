...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(
      `[INFO] Executing DynamoDB Query on table: ${process.env.TABLE_NAME} using userId-index`,
    );
    ...
    console.log(
      "[NE-5] DynamoDB Query StatusCode:",
      response.$metadata.httpStatusCode,
    );
    console.log(
      `[SUCCESS] DynamoDB Query successful. Retrieved ${response.Items.length} tasks for userId: ${userId}`,
    );

    return { statusCode: 200, headers, body: JSON.stringify(response.Items) };
  } catch (error) {
    console.error(
      `[ERROR] Failed to query DynamoDB for userId: ${userId}. Error details:`,
      error,
    );
    return {
      statusCode: 500,
