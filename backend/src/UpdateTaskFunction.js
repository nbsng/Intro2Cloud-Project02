...
exports.handler = async (event) => {
    ...
    console.log(`[INFO] Processing PUT /tasks/${taskId} request for userId: ${userId}`);
    ...
    try {
        ...
        console.log(`[INFO] Executing DynamoDB UpdateItem on table: ${process.env.TABLE_NAME} for taskId: ${taskId}`);
        ...
        console.log("[NE-5] DynamoDB UpdateItem StatusCode:", response.$metadata.httpStatusCode);
        console.log(`[SUCCESS] Task ${taskId} updated for userId: ${userId}`);

        return { statusCode: 200, headers, body: JSON.stringify(response.Attributes) };

    } catch (error) {
        console.error(`[ERROR] Failed to update task ${taskId} in DynamoDB. Error details:`, error);
