...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!body.title || !body.dueDate) {
            console.warn("[WARN] Invalid input: missing required field 'title' or 'dueDate'.");
            return { statusCode: 400, headers, body: JSON.stringify({ error: "Thieu truong bat buoc: title hoac dueDate" }) };
        }
        ...
        console.log(`[INFO] Executing DynamoDB PutItem on table: ${process.env.TABLE_NAME} for taskId: ${taskId}`);
        ...
        console.log("[NE-5] DynamoDB PutItem StatusCode:", response.$metadata.httpStatusCode);
        console.log(`[SUCCESS] Task ${taskId} created for userId: ${userId}`);

        return { statusCode: 201, headers, body: JSON.stringify(newTask) };

    } catch (error) {
        console.error(`[ERROR] Failed to create task in DynamoDB. Error details:`, error);
