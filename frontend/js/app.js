...
async function fetchTasks() {
  ...
  try {
    ...
    if (
      !["Unauthorized", "No token", "Session expired"].includes(error.message)
    ) {
      taskList.innerHTML =
        '<p class="text-red-500 italic">Lỗi tải dữ liệu. Vui lòng thử lại.</p>';
...
function renderTasks(tasks) {
  ...
  if (!tasks || tasks.length === 0) {
    taskList.innerHTML =
      '<p class="text-gray-500 dark:text-gray-400 italic text-center py-4">Chưa có công việc nào.</p>';
    return;
  }
